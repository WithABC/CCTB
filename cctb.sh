...
load_config() {
    ...
    if [[ -n "$token_encoded" ]]; then
        ...
        [[ -n "$token_decoded" ]] && config_content=$(echo "$config_content" | sed "s/^GITHUB_TOKEN=.*/GITHUB_TOKEN=$token_decoded/") || warning "Token解码失败，使用原始编码值（可能无法同步）"
    fi
...
# 同步函数：同步管理菜单
sync_menu() {
    local choice is_github=$([[ "$SYNC_MODE" == "github" ]] && echo 1 || echo 0)
    while true; do
        print_header
        echo -e "\n${BOLD}${gl_blue}同步管理${gl_reset} ${gl_cyan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_reset}"
        echo -e "\n当前模式：${BOLD}$SYNC_MODE${gl_reset}"
        if [[ $is_github -eq 1 ]]; then
            echo -e "\n1. 同步到GitHub    2. 从GitHub同步"
            echo -e "\n3. 切换到本地模式  0. 返回上级菜单"
        else
            echo -e "\n1. 切换到GitHub模式  0. 返回上级菜单"
        fi
...
# 命令管理函数：添加命令（含高危检测+缓存清空）
add_command() {
    local name cmd desc confirm cmd_escaped cmd_type
    local type_options=()
    type_options+=("1) $CMD_TYPE_LOCAL（直接输入命令内容）")
    [[ "$SYNC_MODE" == "github" ]] && type_options+=("2) $CMD_TYPE_PRIVATE_REPO（自动使用已保存GitHub配置）")
    ...
    while true; do
        ...
        case "$cmd_type" in
            ...
            $CMD_TYPE_PRIVATE_REPO)
                ...
                cmd="bash <(curl -s --connect-timeout 10 --max-time 30 -H 'Authorization: token $GITHUB_TOKEN' $private_script_url)"
                ;;
        ...
        clear_cache
        [[ "$SYNC_MODE" == "github" ]] && { echo -e "\n${gl_green}${CLOUD} 自动同步到GitHub...${gl_reset}"; sync_to_github "add_command"; }
...
edit_command() {
    ...
    while true; do
        ...
        [[ "$SYNC_MODE" == "github" ]] && edit_type_options+=("2) $CMD_TYPE_PRIVATE_REPO（自动使用已保存GitHub配置）")
        ...
        if [[ -z "$new_cmd_type_num" ]]; then
            ...
            case "$new_cmd_type_num" in
                ...
                2)
                    [[ "$SYNC_MODE" != "github" ]] && { error "本地模式不支持私人仓库命令"; error_retry; continue; }
        ...
        clear_cache
        [[ "$SYNC_MODE" == "github" ]] && { echo -e "\n${gl_green}${CLOUD} 自动同步到GitHub...${gl_reset}"; sync_to_github "edit_command"; }
        press_any_key_continue
    done
}

# 命令管理函数：删除命令（批量选择+逆序删除+缓存清空）
delete_command() {
    if ! validate_and_reset_commands_file; then
        error_retry
        settings
    ...
    while true; do
        ...
        parsed_nums=$(parse_selection "$num_str" "$cmd_count")
        if [[ -z "$parsed_nums" ]]; then
            error "无效编号！需为 1-$cmd_count 之间的连续/离散数字"
            error_retry
            continue
        fi
        del_cmd_names=()
        ...
        case "$confirm" in
            y|Y|"")
                ...
                if $delete_success; then
                    ...
                    clear_cache
                    [[ "$SYNC_MODE" == "github" ]] && { echo -e "\n${gl_green}${CLOUD} 自动同步到GitHub...${gl_reset}"; sync_to_github "delete_command"; }
                fi
                press_any_key_continue
                ;;
            n|N)
    ...
    while true; do
        ...
        case "$confirm" in
            y|Y|"")
                ...
                clear_cache
                [[ "$SYNC_MODE" == "github" ]] && { echo -e "\n${gl_green}${CLOUD} 自动同步到GitHub...${gl_reset}"; sync_to_github "sort_commands"; }
                press_any_key_continue
                ;;
            n|N)
...
import_commands() {
    ...
    while true; do
        ...
        clear_cache
        [[ "$SYNC_MODE" == "github" ]] && { echo -e "\n${gl_green}${CLOUD} 自动同步到GitHub...${gl_reset}"; sync_to_github; }
        press_any_key_continue
        import_export_menu
    done
}
...
export_quick_connect() {
    while true; do
        ...
        if [[ "$sync_mode" != "github" || -z "$github_repo" || -z "$token_decoded" ]]; then
...
config_menu() {
    ...
    while true; do
        ...
        echo -e "\n1. 查看当前配置    2. 重新配置GitHub"
        echo -e "3. 导出快速连接    4. 修改启动命令"
        echo -e "5. 查看帮助信息    0. 返回上级菜单"
        read -e -p "$(echo -e "\n${gl_blue}选择：${gl_reset}")" choice
        case "$choice" in
            1)
                local current_cmd=$(get_current_cmd_name)
...
show_welcome() {
    ...
    while true; do
        ...
        echo -e "\n${gl_cyan}功能：存储常用命令 | 快速执行 | 多设备同步（GitHub） | 自定义启动命令${gl_reset}"
        ...
        read -e -p "$(echo -e "\n${gl_blue}请选择 [0/1/2]：${gl_reset}")" choice
        case "$choice" in
            1) 
                setup_local_mode; 
                break 
...
show_help() {
    ...
    echo -e "  ${gl_green}$current_cmd -s/--sync${gl_reset}         手动触发GitHub同步（仅GitHub模式有效，自动处理冲突）"
    ...
    echo -e "\n  ${gl_blue}1. Token存储${gl_reset} → GitHub Token采用Base58编码存储于~/.cctb/config，避免明文泄露"
    ...
    echo -e "     - config：同步模式/仓库/Token（Base58编码）/缓存大小配置"
    ...
    echo -e "  ${gl_blue}2. 同步失败${gl_reset} → 检查GitHub仓库存在性、Token权限（contents）、网络连接（curl超时设置10秒）"
...
show_version() {
    ...
    echo -e "安全特性：GitHub Token Base58编码存储 + 高危命令二次确认（含通配符/权限篡改检测）"
...
init_config() {
    ...
    if $is_first_run && [[ ! -f "$CONFIG_FILE" || "$(get_config_value "SYNC_MODE")" != "github" ]]; then
        show_welcome
    fi
    init_cache
}
...
settings() {
    ...
    while true; do
        ...
        local cmd_count=$(echo "$CMD_JSON" | jq -r '.commands | if type == "array" then length else 0 end' 2>/dev/null || echo 0)
        local current_cmd=$(get_current_cmd_name)
        echo -e "\n${BOLD}${gl_cyan}运行状态：$mode_icon $mode_text | 命令总数：$cmd_count | 缓存状态：$( [[ $CACHE_INITED == true ]] && echo "已加载（${#CMD_CACHE[@]}/${CACHE_SIZE}条）" || echo "未加载" ) ${gl_reset}"
        if [[ "$cmd_count" -eq 0 ]]; then
...
init_and_start() {
    ...
    if remote_ver=$(get_remote_version); then
        if version_compare "$local_ver" "$remote_ver"; then
            ...
            update_choice=$(echo "$update_choice" | xargs | tr '[:lower:]' '[:upper:]')
