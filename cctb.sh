...
get_remote_version() {
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        local mac_major=$(echo "$mac_version" | cut -d. -f1)
        local mac_minor=$(echo "$mac_version" | cut -d. -f2)
        if (( mac_major < 10 || (mac_major == 10 && mac_minor < 14) )); then
            stat_cmd="stat -f %c"
        else
            stat_cmd="stat -f %m"
        fi
        ...
        case "$new_cmd_type" in
            ...
            "$CMD_TYPE_NETWORK")
                if [[ -n "$curr_cmd" ]]; then
                    ...
                    curr_net_url=$(echo "$full_url" | sed 's/^https\?:\/\///')
                    [[ -z "$curr_net_url" ]] && curr_net_url="$curr_cmd"
                fi
